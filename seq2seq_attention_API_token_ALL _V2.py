...
from tensorflow.keras.layers import Input, LSTM, Dense, Embedding, Flatten, TimeDistributed, Dropout, LSTMCell, RNN, Bidirectional, Concatenate, BatchNormalization
...
file_path = os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', 'chainsea_all_API_4'))
os.environ["PATH"] += os.pathsep + 'C:/Program Files (x86)/Graphviz2.38/bin/'

class seq2seq_Attention_token_all():
...
test = seq2seq_Attention_token_all('P4_big_BN.h5', 'QA_all_token.txt')
#model = test.train()
model = test.create_model()
encoder_Inference, decoder_Inference = test.createAttentionInference(model)
while True:
    text = input('【input Answer】 \n' )
    result = test.translate(text, encoder_Inference, decoder_Inference, True).replace(" ","")
    print(result)
####
#print(test.translate('歷史建築、古蹟登錄流程辦法', encoder_Inference, decoder_Inference, True))
#
